...
def sort_together(iterables, key_list=(0,), key=None, reverse=False):
    ...
    return list(
        zip(*sorted(zip(*iterables, strict=False), key=key_argument, reverse=reverse), strict=False)
    )
...
class numeric_range(abc.Sequence, abc.Hashable):
    ...
    def __getitem__(self, key):
        if isinstance(key, int):
            ...
            raise TypeError(
                "numeric range indices must be " f"integers or slices, not {type(key).__name__}"
            )
