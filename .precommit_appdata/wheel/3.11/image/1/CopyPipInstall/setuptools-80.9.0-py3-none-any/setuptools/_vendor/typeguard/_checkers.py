...
protocol_check_cache: WeakKeyDictionary[type[Any], dict[type[Any], TypeCheckError | None]] = (
    WeakKeyDictionary()
)
...
def check_self(
    ...
    if isclass(value):
        if not issubclass(value, memo.self_type):
            raise TypeCheckError(
                f"is not an instance of the self type " f"({qualified_name(memo.self_type)})"
            )
